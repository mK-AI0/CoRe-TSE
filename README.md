...
CORE-TSE（Complementary EEG-Speech Representation Learning for EEG-Guided Target Speaker Extraction）是 `Li_ICASSP.pdf` 对应的轻量参考实现。
...
仓库**不包含**数据集、数据预处理、预训练模型、checkpoint、训练日志、TensorBoard 文件、结果 CSV/JSON 或论文表格导出文件。它是参考实现，而不是完整的结果复现包。
...
请自行从原始数据集作者或其官方发布渠道获得数据并完成预处理。运行前必须在命令行显式提供本机 `metadata_path`、`audio_dir`、`eeg_dir` 或 `data_dir`；代码不含任何默认私有路径。
